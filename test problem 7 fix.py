...
def get_date_filter(settings, minute=datetime.now().minute, hour=datetime.now().hour,
    ...
    if minute != '*' and hour != '*':
        ...
        date_filter = datetime(year, month, day, hour,
                               minute).strftime(date_format)
...
def check_match(line, filter_pattern, is_regex, is_casesensitive, is_reverse):
    ...
    if is_regex:
        ...
        check_result = (filter_pattern in line) if is_casesensitive else (
            filter_pattern.lower() in line.lower())
...
def get_web_requests(data, pattern, date_pattern=None, date_keys=None):
    ...
    for request_tuple in requests_dict:
        if date_pattern:
            str_datetime = __get_iso_datetime(
                request_tuple[1], date_pattern, date_keys)
        else:
...
def get_auth_requests(data, pattern, date_pattern=None, date_keys=None):
    ...
    for request_tuple in requests_dict:
        if date_pattern:
            str_datetime = __get_iso_datetime(
                request_tuple[0], date_pattern, date_keys)
        else:
...
def __get_iso_datetime(str_date, pattern, keys):
    ...
    d_datetime = datetime(int(a_date[keys['year']]) if 'year' in keys else __get_auth_year(),
                          months_dict[a_date[keys['month']]], int(
...
class InsightLogAnalyzer:
    ...
    def __init__(self, service, data=None, filepath=None):
        ...
        if filepath:
            ...
            self.filepath = self.__settings['dir_path'] + \
                self.__settings['accesslog_filename']
    ...
    def add_date_filter(self, minute=None, hour=None, day=None, month=None, year=None):
        now = datetime.now()
        ...
        date_filter = get_date_filter(
            self.__settings, minute, hour, day, month, year)
    ...
    def remove_filter(self, index):
        try:
            ...
            raise IndexError(f"No filter at index {index}")
    ...
    def check_all_matches(self, line, filter_patterns):
        ...
        for pattern_data in filter_patterns:
            ...
            to_return = tmp_result if to_return is None else (
                tmp_result and to_return)
        return to_return

    def filter_all(self):
        """
        Apply all defined patterns and yield filtered data line by line.
        Uses a generator for efficiency with large files.
        """
        if self.data:
            source = self.data.splitlines(keepends=True)
            if not self.data.strip():
                # TODO: replace with logging.warning if using logging
                print("Warning: Provided data is empty")
        else:
            try:
                with open(self.filepath, 'r') as file_object:
                    ...
                    if not first_char:
                        ...
                        return
                    file_object.seek(0)  # rewind after peeking
                    source = file_object
